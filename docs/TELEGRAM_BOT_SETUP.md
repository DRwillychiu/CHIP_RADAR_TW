...
`scheduler.ps1` 加了一個 `every=2`(每 2 分鐘)的 job。這是 v3.75.0 為此新增的
排程型態 —— 原本只支援固定時刻,指令要能隨時下,列 720 個時刻不現實。

- **webhook** 需要公開網址,家用電腦沒有
- **常駐 daemon** 反應即時,但多一個要監控、會默默掛掉、要自動重啟的東西
- **輪詢**:`scheduler.ps1` 本來就每分鐘在跑,幾乎零成本。而且它掛掉時整條排程
  也掛了 —— 少一個獨立的故障點

該 job 標了 `quiet=$true`:沒收到指令時完全不寫 log。一天 720 次 × 3 行會把真正
有用的紀錄淹掉。
...
所以輪詢器**絕對不能同步跑長工作**:`/refresh` 要 60-90 秒,萬一壓到 21:17
那一分鐘,當天的 daily-full 就整個不會跑。

拆法:

```
輪詢 (排程同步呼叫)     問一次 getUpdates → 記 offset → 把工作丟出去 → 結束
                        永遠 1 秒內完成,不佔住排程器
   ↓ DETACHED_PROCESS
工人 (--run refresh)     真正的重抓與推送,想跑多久都行
```
...
否則每 2 分鐘都會重看到同一條 `/refresh` 而重複執行。已讀到哪存在 `.tg_poll_offset`。
...
| 鎖檔 | 擋什麼 | 殘骸判定 |
|---|---|---|
| `.tg_poll.lock` | 同時 `getUpdates` —— Telegram 對同一個 bot 會回 **409** | 120 秒(輪詢本來就 1 秒內結束) |
