...
> **版本**:v3.36.2（**15 標籤 + 2 T+1 verified + Level 1/2/3 分層 + 派系 + 實戰信號 + 時間衰減 + 處置持倉資料層**）
...
| v3.36.0 | **B5 處置持倉追蹤**：淨累積 × 處置名單交叉,風險三級 + 快照時間軸,標籤 15→16 |
| v3.36.1 | A 方案實驗:標籤改「處置玩家」只標處置中買進 → 仍 28/29 全中(快照僅 6/4 起 << 60 天窗口) |
| v3.36.2 | **D 方案定案**:處置持倉**不上標籤**(等快照滿 30 天 ~7/4 重評),純資料層 + tab14 面板顯示,標籤回 15 |
| v3.37.0 | **新 tab 15 持股結構**:`margin_maintenance.py` 個股市場估算維持率 (30日均價反推, 健康/警戒/高風險/斷頭) + `tdcc_holdings.py` 集保 400 張大戶 movers (週六 workflow + history snapshot)。**不上標籤** — 純資料層 + 面板顯示 |
...
完整持倉曝險資訊保留在:
- `master_profiles.json` 的 `master.disposal_holdings`（per-master 明細）
- `master_profiles.json` 的 `disposal_holdings_global`（全體曝險排行 cap 30）
- tab14 前端「⛓️ 處置股持倉風險」面板 + detail popup 明細表

#### 為什麼不上標籤(三輪迭代誠實揭露)

| 版本 | 標籤條件 | 實跑結果 | 問題 |
|---|---|---|---|
| v3.36.0 | trapped/high 任一 > 0 | **28/29 全中** | 66 檔處置中含群創/環球晶等權值股,所有大戶都有曝險 → 標籤=噪音 |
| v3.36.1 (A) | bought_during_disposal_count > 0 | **仍 28/29** | 快照 6/4 起只 8 天 << 60 天窗口,6/4 前已處置股 `first_active_date` 都被記為 6/4 → 6/4 後任何買進都判「處置中買進」 |
| v3.36.2 (D) | **暫不上標籤** | — | 標籤是「結論」,沒結論就不下;tab14 面板曝險排序仍極銳利(航海王 123 億 vs 民哥 3.8 億,一眼可分) |

#### 重開計畫(~2026-07-04)

`disposal_history/` 每日快照自動累積。當快照覆蓋整個分析窗口(滿 30 天 ≈ 7/4)後重評,屆時 `first_active_date` 才能準確反映「真正處置生效日」,「處置中買進」判定才有區別力。

#### 計算 + 可信度(資料層仍適用)

| 項 | 內容 |
|---|---|
| **計算** | `disposal_holdings.py`:每 master × 每股窗口內淨累積 = Σ(buy_lot − sell_lot)(**buys + sells 雙榜都算**,出貨日常只在賣超榜)→ 與處置名單交叉 → 快照時間軸判定買進時點 |
| **大額門檻** | 淨累積 ≥100 張 **或** ≥1000 萬(OR,`MIN_NET_LOTS` / `MIN_NET_WAN` 可調) |
| **風險三級** | `active`→⛓️trapped(被鎖)/ `imminent_1`→🚨high(差1次)/ `imminent_2`→👀watch(差2次) |
| **⚠️ 可信度揭露** | **持倉 = 窗口內可見淨累積,非真實庫存(~70-75%)**:(1) TWSE 每分點每日只公布 Top 30 榜 (2) 跨券商賣出不可見 (3) 窗口外舊部位不可見 (4) 快照僅 6/4 起累積。對策:大額才列入 + 輸出買/賣/天數組成供人眼複核;當沖/隔日沖買賣自然互抵 → 被列出的幾乎都是真持倉者(指標自我修正) |
